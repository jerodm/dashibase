      ...
      <div class="bg-white py-8 px-4 shadow-center sm:rounded-lg sm:px-10">
        ...
        <div class="space-y-6 pt-4">
          ...
          <div>
            ...
            <div class="mt-1">
              <input id="password" @keyup.enter="resetPassword" :disabled="loading" v-model="password" name="password" type="password" required class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm" />
            </div>
          </div>
          <!-- Warning -->
          ...
          <!-- Button -->
          <div>
            <button @click="resetPassword" :disabled="loading || success" class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-500 hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
              {{ loading ? 'Loading...' : (success ? 'Redirecting...' : 'Reset password') }}
            </button>
          </div>
        </div>
